...
def process_pair(binner1, binner2, stats_path):
    """TODO: this method is confusing. We need to refactor this to make it
    easier to maintain.
    """
    logging.debug("-".join(["-"] * 20) + f"> Processing pair {binner1} and {binner2}")

    bins1 = get_bins_from_binner(binner1)
    bins1_stats = get_stats(binner1, stats_path)
    bins2 = get_bins_from_binner(binner2)
    bins2_stats = get_stats(binner2, stats_path)

    logging.debug(f"ACA {binner2}")
    ...
    for bin_1 in bins1:
        ...
        for bin_2 in bins2:
            ...
            # choose the highest % ID, dependinsh of which bin is a subset of the other
    ...
    for bin_1 in all_bin_pairs:
        ...
        score = bins1_stats[bin_1][0] - bins1_stats[bin_1][1] * 5 + 0.5 * math.log(N50)
        current_bin = bin_1
        current_contigs = bins1[bin_1]
        current_stats = bins1_stats[bin_1]

        for bin_2 in all_bin_pairs[bin_1]:
            logging.debug(f"compare: >>> {bin_1} with {score}")
            # check for sufficient overlap (80% bin length)
            bins2_matches.append(bin_2)
            # check if this bin is better than original
            N50 = calculate_n50(list(bins2[bin_2].values()))
            logging.debug(bins2_stats)
            if (bins2_stats[bin_2][0] - bins2_stats[bin_2][1] * 5 + 0.5 * math.log(N50)) > score:
                current_bin = bin_2
                current_contigs = bins2[bin_2]
                current_stats = bins2_stats[bin_2]
                logging.debug(
                    f"compare: {bin_2} better then {bin_1} {bins2_stats[bin_2][0] - bins2_stats[bin_2][1] * 5} > {score}"
                )
            elif (bins2_stats[bin_2][0] - bins2_stats[bin_2][1] * 5) == score:
                logging.debug(
                    f"compare: {bin_2} the same as {bin_1} {bins2_stats[bin_2][0] - bins2_stats[bin_2][1] * 5} = {score}"
                )
                if bins1_stats[bin_1][0] > bins2_stats[bin_2][0] and bins1_stats[bin_1][1] < bins2_stats[bin_2][1]:
                    logging.debug(f"compare: {bin_1} better {bin_2} by completeness and contamination")
                elif bins1_stats[bin_1][0] == bins2_stats[bin_2][0] and bins1_stats[bin_1][1] == bins2_stats[bin_2][1]:
                    logging.debug(
                        f"compare: {bin_1} [{len(bins1[bin_1])} contigs] the same as {bin_2} [{len(bins2[bin_2])} contigs] by completeness and contamination"
                    )
                elif bins1_stats[bin_1][0] < bins2_stats[bin_2][0] and bins1_stats[bin_1][1] > bins2_stats[bin_2][1]:
                    logging.debug(f"compare: {bin_2} better {bin_1} by completeness and contamination")
                else:
                    logging.debug(
                        f"compare: difficult case {bin_1} [{len(bins1[bin_1])} contigs] ({bins1_stats[bin_1][0]}, {bins1_stats[bin_1][1]}) vs {bin_2} [{len(bins2[bin_2])} contigs] ({bins2_stats[bin_2][0]}, {bins2_stats[bin_2][1]})"
                    )
            else:
    ...
    untouched_bins2 = list(set(bins_2_stats.keys()).difference(set(bins2_matches)))
...
def main(args):
    ...
    for binner1, binner2 in zip(binners, binners[1:]):
        best_bins, best_stats = process_pair(binner1, binner2, args.stats)
        logging.info(f"best set has {len(best_bins)} bins")
        for bin_ in best_bins:
            logging.debug(f"{bin_} has {len(list(bins[bin_].keys()))} contigs")
    ...
    for bin_ in best_bins:
        for binner in binners:
            if bin_ in os.listdir(binner):
                copy(os.path.join(binner, bin_), os.path.join(consolidated_bins, bin_))
                break
    ...
    contig_mapping = {}
    for bin_ in os.listdir(consolidated_bins):
        bin_path = os.path.join(consolidated_bins, bin_)
        with open(bin_path, "r") as file_in:
            for record in SeqIO.parse(file_in, "fasta"):
                if record.id not in contig_mapping:
                    contig_mapping[record.id] = bin_
                elif best_stats[bin_] > best_stats[contig_mapping[record.id]]:
                    contig_mapping[record.id] = bin_
    ...
    for bin_ in os.listdir(consolidated_bins):
        bin_path = os.path.join(consolidated_bins, bin_)
        drep_bin_path = os.path.join(dereplicated_bins, bin_)
        with open(bin_path, "r") as file_in, open(drep_bin_path, "w") as file_out:
            for record in SeqIO.parse(file_in, "fasta"):
                if contig_mapping[record.id] == bin_:
