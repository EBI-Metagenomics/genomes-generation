...
def parse_args():
    ...
    parser.add_argument(
        "--type", type=str, choices=["proks", "euks"],
    )
...
def propagate_taxonomy_proks(cdb_path: Path, wdb_path: Path, taxonomy_path: Path, output_path: Path) -> None:
    """
    Propagate taxonomy from cluster representatives to all cluster members for prokaryotic wf of GGP.
    ...
    Args:
        ...
        taxonomy_path (Path): Path to the representative taxonomy TSV generated by gtdb_to_ncbi_majority_vote.py
...
def propagate_taxonomy_euks(cdb_path: Path, wdb_path: Path, taxonomy_path: Path, output_path: Path) -> None:
    """
    Propagate taxonomy from cluster representatives to all cluster members for eukaryotic wf of GGP.
    Args:
        cdb_path (Path): Path to the Cdb.csv file from dRep (maps genomes to clusters)
        wdb_path (Path): Path to the Wdb.csv file from dRep (maps clusters to representatives)
        taxonomy_path (Path): Path to the representative taxonomy TSV generated by BAT.
        output_path (Path): Path to write the output TSV
    """
    # Load files
    cdb = pd.read_csv(cdb_path)
    ...
    # Rename column in wdb to match taxonomy file
    rep2cluster = wdb[["cluster", "genome"]].rename(columns={"genome": "# bin"})
    ...
    # Merge all cluster members with cluster2tax
    merged = cdb.merge(
        cluster2tax.drop(columns=["# bin"]),
        left_on="secondary_cluster",
        right_on="cluster",
        how="left"
    )

    # Rename column listing cluster members to "# bin"
    merged = merged.rename(columns={"genome": "# bin"})
...
def main():
    args = parse_args()
    if args.type == "proks":
        propagate_taxonomy_proks(args.cdb, args.wdb, args.taxonomy, args.output)
    else:
        propagate_taxonomy_euks(args.cdb, args.wdb, args.taxonomy, args.output)
