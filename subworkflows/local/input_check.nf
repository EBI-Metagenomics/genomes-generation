...
workflow INPUT_CHECK_ALIGN {
    ...
    emit:
    data                                     // channel: [ val(meta), assembly, [ reads ] ]
...
def create_data_channel(LinkedHashMap row) {
    ...
    if (meta.single_end) {
        fastq_meta = [ meta, file(row.assembly), [ file(row.fastq_1) ] ]
    } else {
        if (!file(row.fastq_2).exists()) {
            exit 1, "ERROR: Please check input samplesheet -> Read 2 FastQ file does not exist!\n${row.fastq_2}"
        }
        fastq_meta = [ meta, file(row.assembly), [ file(row.fastq_1), file(row.fastq_2) ] ]
    }
    return fastq_meta
}
