...
function FilterRuns {
  ...
  --mail-type=ALL \
  --wrap="bash ${REPO_PATH}/scripts/run_filter_script.sh -a $ASSEMBLY_STUDY_ACC -r $READS_STUDY_ACC -c $CATALOGUE_PATH -b '${SCIENTIFIC_NAME}' -f ${ALLOW_METAT}" | awk '{print $NF}')
